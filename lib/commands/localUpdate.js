...
const localUpdate = async (subredditName, options = {}) => {
  ...
  const globRoot = path.resolve(config.dataPath(), subredditName.toLowerCase());
  ...
  await pMap(
    ...
    async (filepath) => {
      ...
      const trimmedFilepath = path.relative(config.dataPath(), filepath);
      progress.tick(trimmedFilepath);
    },
    { concurrency: 50 }
  );
