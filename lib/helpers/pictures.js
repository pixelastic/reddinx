...
module.exports = {
   ...
   **/
  isGalleryPost(data) {
    return !_.chain(data).get('gallery_data').isEmpty().value();
  },
  /**
   ...
   **/
  isPicturePost(data) {
    return !_.chain(data).get('preview').isEmpty().value();
  },
  /**
   ...
   */
  fullUrlGallery(data) {
    const firstPictureId = this.galleryFirstPictureId(data);
    if (!firstPictureId) {
      return false;
    }
  ...
  galleryFirstPictureId(data) {
    // Get all pictures in the gallery
    const allPictureIds = _.chain(data)
      .get('gallery_data.items')
      .map('media_id')
      .value();
    // Get the first valid one
    return _.find(allPictureIds, (pictureId) => {
      return _.get(data, `media_metadata.${pictureId}.status`) === 'valid';
    });
  },
  __imoen: imoen,
};
